...
/* harmony default export */ __webpack_exports__["default"] = (function () {
  ...
  _skpm_dialog__WEBPACK_IMPORTED_MODULE_2___default.a.showOpenDialog(document, {
    filters: [{
      name: 'JSON',
      extensions: ['json']
    }],
    ...
    var rawdata = Object(_skpm_fs__WEBPACK_IMPORTED_MODULE_3__["readFileSync"])(filepath);

    if (!rawdata) {
      sketch__WEBPACK_IMPORTED_MODULE_0__["UI"].message('⚠️ File could not be read');
    }

    var json = JSON.parse(rawdata);

    if (!json) {
      sketch__WEBPACK_IMPORTED_MODULE_0__["UI"].message('⚠️ JSON could not be parsed');
    }

    var exportPresets = jsonToExportPref(json);

    if (!exportPresets) {
      sketch__WEBPACK_IMPORTED_MODULE_0__["UI"].message('⚠️ Invalid JSON structure');
      return;
    }

    var persistentAssetCollection = MSPersistentAssetCollection.sharedGlobalAssets();
    var selectionIndex = persistentAssetCollection.exportPresets().count();
    persistentAssetCollection.addExportPresets(exportPresets);
    sketch__WEBPACK_IMPORTED_MODULE_0__["UI"].message('✅ Successfully added Export Presets'); // Open preference window

    var prefController = MSPreferencesController.sharedController();
    var preferencePane;
    ...
    } catch (_unused) {} // don't need to refresh views it since its not cached
...
function jsonToExportPref(json) {
  var exportPresets = NSMutableArray.array();
